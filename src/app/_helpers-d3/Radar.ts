...
export default function Radar(id: any, data: any, options: any, scaleType: any = 'x') {
  let cfg: any = {
  ...
  let allAxis: any = data[0].map(function (i: any, j: any) {
    return i.axis;
  }), //Names of each axis
  ...
  let svg = d3
    ...
    .attr(
      'viewBox',
      `0 0 ${cfg.w + cfg.margin.left + cfg.margin.right} ${cfg.h + cfg.margin.top + cfg.margin.bottom
      }`
    )
    .attr('class', 'radar' + id);
  //Append a g element
